...
First we need to 
1)divide the sentence into words(Use separator space); 
2)divide each word into characters(Use separator empty string); 
3)merge each characters with the specified sp; 
4)merge all the words(Use separator space) and return it.
...
const splitAndMerge = (str, sp) => {
  ...
  for (let i = 0; i < words.length; i++) {
    const el = words[i].split("").join(sp);
    arr.push(el);
  }
  return arr.join(" ");
}
